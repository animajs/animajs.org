define("anima-app/1.0.0/index-debug", ["anima-yocto/1.0.1/index-debug", "anima-underscore/1.0.0/index-debug", "anima-scroll/1.0.0/index-debug", "anima-yocto/1.0.0/index-debug", "anima-fx/1.0.0/index-debug"], function (require, exports, module) {
  var $ = window.$ = require("anima-yocto/1.0.1/index-debug");
  var _ = require("anima-underscore/1.0.0/index-debug");
  var Scroll = require("anima-scroll/1.0.0/index-debug");
  var Fx = require("anima-fx/1.0.0/index-debug");
  ...
  var App = function (element, options, callback) {
    ...
    function Navbar(wrapEl, options) {
      ...
      this.animWrapEl.innerHTML = _.substitute(options.tpl.title, {
        title: ""
      });
    ...
    _.extend(Navbar.prototype, {
      setTitle: function (title) {
        if (typeof title === 'string') {
          this.animWrapEl.innerHTML = _.substitute(this.options.tpl.title, {
            title: title
          });
        }
      },
    ...
    // History监听开始
    app.History.start();
    app.options.initPage && (app.History.navigate(app.options.initPage, {
      replace: true
    }))
    //Return instance
    return app;
  ...
  App.prototype.device = (function () {
    ...
    device.webView && (
    // iPhone 5
    (windowWidth === 320 && windowHeight === 568) || (windowWidth === 568 && windowHeight === 320) ||
    // iPhone 4
    (windowWidth === 320 && windowHeight === 480) || (windowWidth === 480 && windowHeight === 320) ||
    // iPad
    (windowWidth === 768 && windowHeight === 1024) || (windowWidth === 1024 && windowHeight === 768))) {
    ...
    // Add html classes
    if (device.os) {
      var className = device.os + ' ' + device.os + '-' + device.osVersion.split('.')[0] + ' ' + device.os + '-' + device.osVersion.replace(/\./g, '-');
      if (device.os === 'ios') {
